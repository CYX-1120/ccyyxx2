...
def get_dataset(args):
    def _normalize_voc_root(root):
        root = os.path.abspath(root)
        tail = os.path.join('VOCdevkit', 'VOC2007')
        if root.endswith(tail):
            return root[:-len(tail)-1]
        if root.endswith('VOCdevkit'):
    ...
    if args.data_set in ('MS-COCO'):
        data_dir = opj(args.data_root, 'MSCOCO2014')
        ...
        data_dir = _normalize_voc_root(args.data_root)
        os.makedirs(os.path.join(data_dir, 'VOCdevkit', 'files'), exist_ok=True)
