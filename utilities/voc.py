...
class VOC2007(Dataset):
    def __init__(self, root, phase, transform=None):
        # root应该指向包含VOC2007文件夹的目录
        self.root = os.path.abspath(root)
        self.path_devkit = os.path.join(self.root, 'VOCdevkit', 'VOC2007')
        self.path_images = os.path.join(self.root, 'VOCdevkit', 'VOC2007', 'JPEGImages')
        self.phase = phase
        self.transform = transform
        # 移除download_voc2007(self.root)

        # define path of csv file
        path_csv = os.path.join(self.root, 'VOCdevkit', 'files', 'VOC2007')
    ...
    def __getitem__(self, index):
        # image: 3, h, w
        # target: nc; \in {-1, +1, 0}^{nc} where 0 is difficult sample
        # filename: xx.jpg
        ...
        data = {'image': img, 'target': target, 'name': filename}
...
class VOC2012(Dataset):
    def __init__(self, root, phase, transform=None, lab_emb_file=None):
        # root应该指向包含VOC2012文件夹的目录
        self.root = os.path.abspath(root)
        self.path_devkit = os.path.join(self.root, 'VOCdevkit', 'VOC2012')
        self.path_images = os.path.join(self.root, 'VOCdevkit', 'VOC2012', 'JPEGImages')
        self.phase = phase
        self.transform = transform
        # 移除download_voc2012(self.root)

        # define path of csv file
        path_csv = os.path.join(self.root, 'VOCdevkit', 'files', 'VOC2012')
        # define filename of csv file
        file_csv = os.path.join(path_csv, 'classification_' + phase + '.csv')
        ...
        import pickle
        if lab_emb_file and os.path.exists(lab_emb_file):
            with open(lab_emb_file, 'rb') as f:
                self.lab_emb = pickle.load(f)
        else:
            self.lab_emb = torch.randn(len(object_categories), 300)
    ...
    def __getitem__(self, index):
        ...
        data = {'image':img, 'lab_emb': self.lab_emb, 'name': filename, 'target': target}
        return data
...
if __name__ == '__main__':
    ...
    Dset = VOC2007(root=r'E:/PhD/Data_set/ImageSet/VOC2007', phase='test', transform=transform)
