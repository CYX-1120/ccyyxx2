    ...
    def on_forward(self, inputs, targets, file_name, is_train,):
        ...
        if is_train:
            with autocast(enabled=not self.args.disable_amp):
                ...
                # Pass mix_ind to loss function for adaptive weighting
                mix_ind = flag.get('mix_ind', None) if isinstance(flag, dict) else None
                loss = self.loss_fn(outputs, targets, mix_ind=mix_ind)
