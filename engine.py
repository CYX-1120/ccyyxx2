...
class Engine(object):
    ...
    def init(self):
        ...
        # Enable find_unused_parameters if backbone freezing is used
        freeze_epochs = getattr(self.args, 'freeze_backbone_epochs', 0)
        self.model = nn.parallel.DistributedDataParallel(
            self.model, 
            device_ids=[self.rank],
            find_unused_parameters=(freeze_epochs > 0)
        )
        if 'SpliceMix' in self.args.mixer:
            mixer_obj = SpliceMix.SpliceMix(mode=self.args.mixer, grids=self.args.grids,
                                             n_grids=self.args.n_grids, mix_prob=self.args.Sprob)
            # V2: Set dataset for correct cooccur file loading
            if hasattr(mixer_obj, 'set_dataset'):
                mixer_obj.set_dataset(self.args.data_set)
            self.mixer = mixer_obj.mixer
    ...
    def train(self):
        ...
        # Freeze backbone if specified (for fine-tuning pretrained models)
        freeze_epochs = getattr(self.args, 'freeze_backbone_epochs', 0)
        if freeze_epochs > 0:
            self._set_backbone_frozen(True)
            self.logger.info(f"[Freeze] Backbone frozen for first {freeze_epochs} epochs (fine-tuning mode)")
        
        for epoch in range(self.args.start_epoch, self.args.epochs+1):
            # Unfreeze backbone after freeze_epochs
            if freeze_epochs > 0 and epoch == freeze_epochs + 1:
                self._set_backbone_frozen(False)
                self.logger.info(f"[Unfreeze] Backbone unfrozen at epoch {epoch}")
    ...
    def on_start_epoch(self, epoch):
        ...
        # Update mixer's curriculum learning epoch info
        if 'SpliceMix' in self.args.mixer and hasattr(self, 'mixer'):
            # Get the SpliceMix object (mixer is bound method, __self__ is the object)
            mixer_obj = getattr(self.mixer, '__self__', None)
            if mixer_obj is not None:
                mixer_obj.current_epoch = epoch
                mixer_obj.total_epochs = self.args.epochs
    ...
    def load_checkpoint(self):
        ...
        # Handle checkpoints lacking evaluation metrics (e.g., pure pretrained weights)
        mAP_best = checkpoint.get('mAP_best', -1.)
        mAP_curr = checkpoint.get('mAP_curr', -1.)
        loss_val_best = checkpoint.get('loss_val_best', -1.)
        loss_tr = checkpoint.get('loss_tr', -1.)
        epoch_ckpt = checkpoint.get('epoch', -1)

        self.logger.info(f"precision_test_best: {mAP_best:.4f}, "
                         f"precision_test_curr: {mAP_curr:.4f}, "
                         f"loss_test_best: {loss_val_best:.4f}, "
                         f"loss_train: {loss_tr:.4f} in epoch {epoch_ckpt}, "
    ...
    def _set_backbone_frozen(self, freeze: bool):
        ...
        # Access backbone through DDP wrapper if needed
        model = self.model.module if hasattr(self.model, 'module') else self.model
        if hasattr(model, 'backbone'):
            for param in model.backbone.parameters():
                param.requires_grad = not freeze
            status = "frozen" if freeze else "unfrozen"
            self.logger.info(f"[Backbone] {status}, trainable params: {sum(p.numel() for p in model.parameters() if p.requires_grad)}")
        else:
            self.logger.warning("[Backbone] No 'backbone' attribute found in model, cannot freeze.")
